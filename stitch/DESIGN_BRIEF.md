...
- LoopCheck Status Board turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page app called LoopCheck Status Board. This is a simple web utility, not a browser game. It should show three compact status cards, a refresh button that updates a visible timestamp, and one toggle that changes a status label between Ready and Paused. Keep it simple and use the existing…
...
- Status Utility - LoopCheck Status Board

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Status Utility - LoopCheck Status Board
